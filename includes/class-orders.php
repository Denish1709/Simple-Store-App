...
class Orders
{
    ...
    )
    {
        ...
        $response = callAPI(
            ...
            'POST',
            [
                'collection_id' => BILLPLS_COLLECTION_ID,
                'email' => $_SESSION['user']['email'],
                'name' => $_SESSION['user']['email'],
                'amount' => $total_amount * 100,
                'callback_url' => 'http://simple-store-app.local/payment-callback',
                'description' => 'Order #' . $order_id, //Order #3,
                'redirect_url' => 'http://simple-store-app.local/payment-verification'
            ],
            [
            ...
            // Step 5: if the response is successful, update the order with bill id ($response->id)
